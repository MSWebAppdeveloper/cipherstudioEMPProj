...
const notificationLogController = {
  ...
  getNotificationLogById: async (req, res) => {
    try {
      ...
      const notificationLog = await db.notificationLog.findByPk(
        notificationLogId
      );
      if (!notificationLog) {
        return res
          .status(404)
          .json({ error: "Notification log entry not found" });
      }
  ...
  updateNotificationLog: async (req, res) => {
    try {
      ...
      let notificationLog = await db.notificationLog.findByPk(
        notificationLogId
      );
      if (!notificationLog) {
        return res
  ...
  deleteNotificationLog: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Notification log entry deleted successfully" });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: "Internal Server Error" });
    }
